"""Period locking, audit, and accounting-policy services."""
...
class GovernanceManager:
    ...
    def get_or_create_period(self, activity_date, actor_username="system"):
        ...
        success, period_id = self.db.execute(
            """INSERT INTO Accounting_Periods (annee, mois, status, opened_by)
               VALUES (%s, %s, 'OPEN', %s)""",
            (activity_date.year, activity_date.month, actor_username),
        )
        if not success:
    ...
    def assert_writable_period(self, activity_date, actor_username="system"):
        period = self.get_or_create_period(activity_date, actor_username)
    ...
    def record_audit(self, actor_username, action_code, entity_type, entity_id=None, period_id=None, old_values=None, new_values=None, reason=None):
        success, _ = self.db.execute(
            ...
            (
                actor_username or "system", action_code, entity_type,
    ...
    def close_period(self, period_id, actor_username, close_note=""):
        if not self.has_role(actor_username, self.CLOSE_ROLES):
        ...
        success, _ = self.db.execute(
            """UPDATE Accounting_Periods
               ...
               WHERE id_period = %s""",
            (actor_username, close_note, period_id),
        )
        if success:
            self.record_audit(actor_username, "PERIOD_CLOSED", "Accounting_Period", period_id, period_id, new_values={"status": "CLOSED"}, reason=close_note)
        return success

    def reopen_period(self, period_id, actor_username, reason):
        if not reason or not reason.strip():
            raise ValueError("A reason is required to reopen a period.")
        if not self.has_role(actor_username, self.ADMIN_ROLES):
        ...
        if success:
            self.db.execute(
                ...
                (period_id, actor_username, reason.strip(), actor_username),
            )
            self.record_audit(actor_username, "PERIOD_REOPENED", "Accounting_Period", period_id, period_id, new_values={"status": "REOPENED"}, reason=reason.strip())
        return success
